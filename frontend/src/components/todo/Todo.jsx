...
const Todo = () => {
  ...
  const submit = async () => {
    if (Inputs.title === "" || Inputs.body === "") {
      ...
      if (id) {
        await axios
          .post(`${process.env.REACT_APP_API_URL}/api/v2/addTask`, {
            title: Inputs.title,
            body: Inputs.body,
            id: id,
          })
  ...
  const del = async (Cardid) => {
    if (id) {
      await axios
        .delete(`${process.env.REACT_APP_API_URL}/api/v2/deleteTask/${Cardid}`, {
          data: { id: id },
        })
        .then(() => {
  ...
  useEffect(() => {
    if (id) {
      const fetch = async () => {
        await axios
          .get(`${process.env.REACT_APP_API_URL}/api/v2/getTasks/${id}`)
          .then((response) => {
